...
class LeftLeaningRedBlackBST {
    ...
    public:
        ...
        Node *left, *right;
        Node(int key = -1, int val = -1, bool isRed = true, Node *left = nullptr, Node *right = nullptr)
            : key(key), val(val), isRed(isRed), left(left), right(right) {
    ...
    void flipColors(Node *root) {
        ...
        if (root->left != nullptr) root->left->isRed = !root->left->isRed;
        if (root->right != nullptr) root->right->isRed = !root->right->isRed;
    }
    ...
    private: Node *put(Node *root, int key, int val) {
        ...
        // ------------------ NEW CODE FOR BST [w.r.t normal BST] ---------------------//
...
// TODO: deletion not tested
// FOR TESTING
int main() {
    LeftLeaningRedBlackBST bst;

    vector<int> arr  = {8, 2, 0, 7, 1, 3, 9, 5, 6, 4};
    for (int x: arr) {
        bst.put(x, x);
        if (bst.size() > 2 && bst.height() > 10 * log2(bst.size())) {
            int h = bst.height();
            int size = bst.size();
            int logSize = log2(bst.size());
            throw new runtime_error("\nheight became more than 2 logN");
        }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (bst.size() > 2 && bst.height() > 2 * log2(bst.size())) {
            int h = bst.height();
            int size = bst.size();
            int logSize = log2(bst.size());
            throw new runtime_error("\nheight became more than 2 logN");
    ...
    while (bst.size()) {
        ...
        int keyToDelete = bst.select(bst.size() - 1); // k-th smallest element in bst
        bst.remove(keyToDelete);
